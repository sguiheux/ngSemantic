...
var SemanticSelectComponent = (function () {
    ...
    SemanticSelectComponent.prototype.ngAfterViewInit = function () {
        ...
        jQuery(this.select.nativeElement)
            .dropdown(options);
        var searchInput = jQuery('input.search', this.select.nativeElement.parentElement);
        if (searchInput && searchInput.length === 1) {
            jQuery(searchInput[0]).on('keydown', function (event) {
                setTimeout(function () {
                    _this.onSearchChange.emit(event.target.value);
                }, 1);
            });
        }
    };
    ...
    __decorate([
        core_1.Output(),
        __metadata("design:type", core_1.EventEmitter)
    ], SemanticSelectComponent.prototype, "onSearchChange", void 0);
    __decorate([
        core_1.ViewChild("select", { static: true }),
